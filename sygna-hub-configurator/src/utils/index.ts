...
export const genConfigYamlString = (config: object) => {
  ...
  for (const sectionKey in comments) {
    ...
    for (const key in comments[sectionKey]) {
      if (key === 'comment') {
        sectionNode.commentBefore = `${formatComment(comments[sectionKey].comment)}`;
      } else {
        const keyNode = sectionNode.get(key, true);
        if (keyNode) {
          keyNode.commentBefore = `${formatComment(comments[sectionKey][key])}`;
        }
      }      
    }
  }
